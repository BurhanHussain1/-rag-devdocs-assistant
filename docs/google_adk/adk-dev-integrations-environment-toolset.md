---
url: https://adk.dev/integrations/environment-toolset/
title: Environment Toolsets for ADK
...
* **working\_dir**: (optional) The directory where the agent will perform
...
* **env\_vars**: (optional) A dictionary of environment variables to be set
  for the execution context.
* **max\_output chars**: (optional) A parameter to use with `EnvironmentToolset` to limit the maximum number of characters returned from file reads or command executions. This helps prevent large file contents or command outputs from exceeding the agent's context window limit.
