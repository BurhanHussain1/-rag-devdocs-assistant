---
url: https://adk.dev/deploy/agent-runtime/test/
title: Test deployed agents in Agent Runtime
...
1. In your development environment, make sure you are authenticated to
   ...
   ```
2. With the Project ID you used for deployment, run this command to get
...
1. In a terminal window of your development environment, create a session
   ...
   curl \
       ...
       -d '{"class_method": "async_create_session", "input": {"user_id": "u_123"},}'
   ```
2. In the response from the previous command, extract the created **Session ID**
   from the **id** field:
