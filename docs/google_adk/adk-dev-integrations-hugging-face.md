---
url: https://adk.dev/integrations/hugging-face/
title: Hugging Face MCP tool for ADK
...
* `DEFAULT_HF_TOKEN`: ⚠️ Requests are serviced with the `HF_TOKEN` received in
  the Authorization: Bearer header. The DEFAULT\_HF\_TOKEN is used if no header
...
* `SEARCH_ENABLES_FETCH`: When set to true, automatically enables the
  hf\_doc\_fetch tool whenever hf\_doc\_search is enabled
