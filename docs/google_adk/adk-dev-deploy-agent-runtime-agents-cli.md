...
title: Deploy to Agent Runtime with Agents CLI
...
2. Run the Agents CLI `scaffold enhance` command to add the files required for deployment into
   your project.

   ```
   agents-cli scaffold enhance --deployment-target agent_engine
   ```
3. Follow the instructions from the Agents CLI tool. In general, you can accept
