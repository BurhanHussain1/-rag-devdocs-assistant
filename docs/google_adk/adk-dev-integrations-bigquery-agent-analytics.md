...
title: BigQuery Agent Analytics plugin for ADK
...
existing tables), **Tool Provenance** tracking (LOCAL, MCP, SUB\_AGENT, A2A,
TRANSFER\_AGENT, TRANSFER\_A2A), **HITL Event Tracing** for human-in-the-loop
...
project ". These actions create events which are recorded
...
| **span\_id** | `STRING` | `NULLABLE` | 16-character hex Span ID identifying this specific atomic operation. **Tracked on the plugin's internal stack, not exported as an OTel span** — the plugin does not call `tracer.start_span` against your configured OpenTelemetry provider. The root invocation span reuses the ambient OTel span's id when one is active; child spans are generated internally (see [Tracing and observability](#tracing-and-observability)). | `3916f5762bcd4d42` |
