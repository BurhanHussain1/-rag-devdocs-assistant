...
Use [`Runner`](../ref/run/#agents.run.Runner "Runner") to execute the agent and get a [`RunResult`](../ref/result/#agents.result.RunResult "RunResult
...
For a second turn, you can either pass `result.to_input_list()` back into `Runner.run(...)`, attach a [session](../sessions/), or reuse OpenAI server-managed state with `conversation_id` / `previous_response_id`. The [running agents](../running_agents/) guide compares these approaches.
...
| The SDK to load and save history for you | [`session=...`](../sessions/) |
| OpenAI-managed server-side continuation | `previous_response_id` or `conversation_id` |

For the tradeoffs and exact behaviors, see [Running agents](../running_agents/#choose-a-memory-strategy).

Use a plain `Agent` plus `Runner` when the task mainly lives in prompts, tools, and conversation state. If the agent should inspect or modify real files in an isolated workspace, jump to the [Sandbox agents quickstart](../sandbox_agents/).
...
This quickstart continues with **handoffs** because it is the shortest first example. For the manager-style pattern, see [Agent orchestration](../multi_agent/) and [Tools: agents as tools](../tools/#agents-as-tools).
...
* Learn about [running agents](../running_agents/) and [sessions](../sessions/).
* Learn about [Sandbox agents](../sandbox_agents/) if the work should happen inside a real workspace.
