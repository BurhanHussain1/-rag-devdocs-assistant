---
url: https://openai.github.io/openai-agents-python/realtime/quickstart/
title: Quickstart
framework: openai
---
...
The Python SDK does **not** provide a browser WebRTC transport. This page only covers Python-managed realtime sessions over server-side WebSockets. Use this SDK for server-side orchestration, tools, approvals, and telephony integrations. See also [Realtime transport](../transport/).
...
`session.send_message()` accepts either a plain string or a structured realtime message. For raw audio chunks, use [`session.send_audio()`](../../ref/realtime/session/#agents.realtime.session.RealtimeSession.send_audio "send_audio
...
* SIP / telephony attach flows. See [Realtime transport](../transport/) and the [SIP section](../guide/#sip-and-telephony).
...
For manual turn control, use a raw `session.update` / `input_audio_buffer.commit` / `response.create` flow as described in the [Realtime agents guide](../guide/#manual-response-control).

For the full schema, see [`RealtimeRunConfig`](../../ref/realtime/config/#agents.realtime.config.RealtimeRunConfig) and [`RealtimeSessionModelSettings`](../../ref/realtime/config/#agents.realtime.config.RealtimeSessionModelSettings).
...
When connecting to Azure OpenAI, pass a GA Realtime endpoint URL in `model_config["url"]` and explicit headers. Avoid the legacy beta path (`/openai/realtime?api-version=...`) with realtime agents. See the [Realtime agents guide](../guide/#low-level-access-and-custom-endpoints) for details.

## Next steps

* Read [Realtime transport](../transport/) to choose between server-side WebSocket and SIP.
* Read the [Realtime agents guide](../guide/) for lifecycle, structured input, approvals, handoffs, guardrails, and low-level control.
