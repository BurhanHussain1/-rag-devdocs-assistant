...
`needs_approval` is available on [`function_tool`](../ref/tool/#agents.tool.function_tool "function_tool"), [`Agent.as_tool`](../ref/agent/#agents.agent.Agent.as_tool "as_tool"), [`ShellTool`](../ref/tool/#agents.tool.ShellTool "ShellTool

  
      dataclass
  "), and [`ApplyPatchTool`](../ref/tool/#agents.tool.ApplyPatchTool "ApplyPatchTool

  
      dataclass
  "). Local MCP servers also support approvals through `require_approval` on [`MCPServerStdio`](../ref/mcp/server/#agents.mcp.server.MCPServerStdio "MCPServerStdio"), [`MCPServerSse`](../ref/mcp/server/#agents.mcp.server.MCPServerSse "MCPServerSse"), and [`MCPServerStreamableHttp`](../ref/mcp/server/#agents.mcp.server.MCPServerStreamableHttp "MCPServerStreamableHttp"). Hosted MCP servers support approvals via [`HostedMCPTool`](../ref/tool/#agents.tool.HostedMCPTool "HostedMCPTool

  
      dataclass
  ") with `tool_config={"require_approval": "always"}` and an optional `on_approval_request` callback. Shell and apply\_patch tools accept an `on_approval` callback if you want to auto-approve or auto-reject without surfacing an interruption.
...
2. If an approval decision for that tool call is already stored in the [`RunContextWrapper`](../ref/run_context/#agents.run_context.RunContextWrapper "RunContextWrapper

     
         dataclass
     "), the runner proceeds without prompting. Per-call approvals are scoped to the specific call ID; pass `always_approve=True` or `always_reject=True` to persist the same decision for future calls to that tool during the rest of the run.
3. Otherwise, execution pauses and `RunResult.interruptions` (or `RunResultStreaming.interruptions`) contains [`ToolApprovalItem`](../ref/items/#agents.items.ToolApprovalItem "ToolApprovalItem

     
         dataclass
     ") entries with details such as `agent.name`, `tool_name`, and `arguments`. This includes approvals raised after a handoff or inside nested `Agent.as_tool()` executions.
...
* Run-wide fallback: set [`RunConfig.tool_error_formatter`](../ref/run/#agents.run.RunConfig.tool_error_formatter "tool_error_formatter

    
        class-attribute
        instance-attribute
    ") to control the default model-visible message for approval rejections across the whole run.
...
* Local [`ShellTool`](../ref/tool/#agents.tool.ShellTool "ShellTool
    ...
    ") and [`ApplyPatchTool`](../ref/tool/#agents.tool.ApplyPatchTool "ApplyPatchTool
        ...
        dataclass
    ") can use `on_approval` to approve or reject immediately in code.
* [`HostedMCPTool`](../ref/tool/#agents.tool.HostedMCPTool "HostedMCPTool

    
        dataclass
    ") can use `tool_config={"require_approval": "always"}` together with `on_approval_request` for the same kind of programmatic decision.
* Plain [`function_tool`](../ref/tool/#agents.tool.function_tool "function_tool") tools and [`Agent.as_tool()`](../ref/agent/#agents.agent.Agent.as_tool "as_tool") use the manual interruption flow on this page.

When these callbacks return a decision, the run continues without pausing for a human response. For Realtime and voice session APIs, see the approval flow in the [Realtime guide](../realtime/guide/).

## Streaming and sessions

The same interruption flow works in streaming runs. After a streamed run pauses, keep consuming [`RunResultStreaming.stream_events()`](../ref/result/#agents.result.RunResultStreaming.stream_events "stream_events

  
      async
  ") until the iterator finishes, inspect [`RunResultStreaming.interruptions`](../ref/result/#agents.result.RunResultStreaming.interruptions "interruptions

  
      class-attribute
      instance-attribute
  "), resolve them, and resume with [`Runner.run_streamed(...)`](../ref/run/#agents.run.Runner.run_streamed "run_streamed

  
      classmethod
  ") if you want the resumed output to keep streaming. See [Streaming](../streaming/) for the streamed version of this pattern.

If you are also using a session, keep passing the same session instance when you resume from `RunState`, or pass another session object that points at the same backing store. The resumed turn is then appended to the same stored conversation history. See [Sessions](../sessions/) for the session lifecycle details.
...
* **Local shell and apply\_patch tools**: `ShellTool` and `ApplyPatchTool` also support `needs_approval`. Use `state.approve(interruption, always_approve=True)` or `state.reject(..., always_reject=True)` to cache the decision for future calls. For automatic decisions, provide `on_approval` (see `examples/tools/shell.py`); for manual decisions, handle interruptions (see `examples/tools/shell_human_in_the_loop.py`). Hosted shell environments do not support `needs_approval` or `on_approval`; see the [tools guide](../tools/).
...
* **Realtime agents**: The realtime demo exposes WebSocket messages that approve or reject tool calls via `approve_tool_call` / `reject_tool_call` on the `RealtimeSession` (see `examples/realtime/app/server.py` for the server-side handlers and [Realtime guide](../realtime/guide/#tool-approvals) for the API surface).
