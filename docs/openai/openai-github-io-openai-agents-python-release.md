...
In this version, [`MCPServer.list_tools()`](../ref/mcp/server/#agents.mcp.server.MCPServer "MCPServer") has two new params: `run_context` and `agent`. You'll need to add these params to any classes that subclass `MCPServer`.