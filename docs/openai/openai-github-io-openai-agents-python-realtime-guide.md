...
If you want the default Python path, read the [quickstart](../quickstart/) first. If you are deciding whether your app should use server-side WebSocket or SIP, read [Realtime transport](../transport/). Browser WebRTC transport is not part of the Python SDK.
...
See [`RealtimeRunConfig`](../../ref/realtime/config/#agents.realtime.config.RealtimeRunConfig) and [`RealtimeSessionModelSettings`](../../ref/realtime/config/#agents.realtime.config.RealtimeSessionModelSettings) for the full typed surface.

## Inputs and outputs

### Text and structured user messages

Use [`session.send_message()`](../../ref/realtime/session/#agents.realtime.session.RealtimeSession.send_message "send_message

  
      async
  ") for plain text or structured realtime messages.
...
Use [`session.send_audio()`](../../ref/realtime/session/#agents.realtime.session.RealtimeSession.send_audio "send_audio

  
      async
  ") to stream raw audio bytes:
...
When a completed model response includes usage, the OpenAI realtime model emits a [`RealtimeModelUsageEvent`](../../ref/realtime/model_events/#agents.realtime.model_events.RealtimeModelUsageEvent "RealtimeModelUsageEvent
...
The session also adds each response's usage to the shared [`RunContextWrapper.usage`](../../ref/run_context/#agents.run_context.RunContextWrapper.usage "usage

  
      class-attribute
      instance-attribute
  "). Read it from `event.info.context.usage` on a subsequent high-level event such as `agent_end` to inspect cumulative usage for the live session.
...
In low-latency local playback, the default playback tracker is often enough. In remote or delayed playback scenarios, especially telephony, use [`RealtimePlaybackTracker`](../../ref/realtime/model/#agents.realtime.model.RealtimePlaybackTracker "RealtimePlaybackTracker") so interruption truncation is based on actual playback progress rather than assuming all generated audio has already been heard.
...
For a concrete server-side approval loop, see [`examples/realtime/app/server.py`](https://github.com/openai/openai-agents-python/tree/main/examples/realtime/app/server.py). The human-in-the-loop docs also point back to this flow in [Human in the loop](../../human_in_the_loop/).
...
The Python SDK includes a first-class SIP attach flow via [`OpenAIRealtimeSIPModel`](../../ref/realtime/openai_realtime/#agents.realtime.openai_realtime.OpenAIRealtimeSIPModel "OpenAIRealtimeSIPModel").
