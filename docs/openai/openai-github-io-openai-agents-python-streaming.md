...
Computer-tool raw events keep the same preview-vs-GA distinction as stored results. Preview flows stream `computer_call` items with one `action`, while `gpt-5.5` can stream `computer_call` items with batched `actions[]`. The higher-level [`RunItemStreamEvent`](../ref/stream_events/#agents.stream_events.RunItemStreamEvent "RunItemStreamEvent

  
      dataclass
  ") surface does not add a special computer-only event name for this: both shapes still surface as `tool_called`, and the screenshot result comes back as `tool_output` wrapping a `computer_call_output` item.
...
Streaming is compatible with runs that pause for tool approval. If a tool requires approval, `result.stream_events()` finishes and pending approvals are exposed in [`RunResultStreaming.interruptions`](../ref/result/#agents.result.RunResultStreaming.interruptions "interruptions

  
      class-attribute
      instance-attribute
  "). Convert the result to a [`RunState`](../ref/run_state/#agents.run_state.RunState "RunState

  
      dataclass
  ") with `result.to_state()`, approve or reject the interruption, and then resume with `Runner.run_streamed(...)`.
...
For a full pause/resume walkthrough, see the [human-in-the-loop guide](../human_in_the_loop/).

## Cancel streaming after the current turn

If you need to stop a streaming run in the middle, call [`result.cancel()`](../ref/result/#agents.result.RunResultStreaming.cancel "cancel"). By default this stops the run immediately. To let the current turn finish cleanly before stopping, call `result.cancel(mode="after_turn")` instead.
...
If you are manually continuing from [`result.to_input_list(mode="normalized")`](../ref/result/#agents.result.RunResultBase.to_input_list "to_input_list"), and `cancel(mode="after_turn")` stops after a tool turn, continue that unfinished turn by rerunning `result.last_agent` with that normalized input instead of appending a fresh user turn right away.
...
- Use [`RunConfig.session_input_callback`](../ref/run/#agents.run.RunConfig.session_input_callback "session_input_callback
