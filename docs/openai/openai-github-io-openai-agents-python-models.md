...
If an agent includes [`ComputerTool`](../ref/tool/#agents.tool.ComputerTool "ComputerTool

  
      dataclass
  "), the effective model on the actual Responses request determines which computer-tool payload the SDK sends. Explicit `gpt-5.5` requests use the GA built-in `computer` tool, while explicit `computer-use-preview` requests keep the older `computer_use_preview` payload.
...
With a registered [`ComputerTool`](../ref/tool/#agents.tool.ComputerTool "ComputerTool

  
      dataclass
  "), `tool_choice="computer"`, `"computer_use"`, and `"computer_use_preview"` are normalized to the built-in selector that matches the effective request model. If no `ComputerTool` is registered, those strings continue to behave like ordinary function names.

Preview-compatible requests must serialize `environment` and display dimensions up front, so prompt-managed flows that use a [`ComputerProvider`](../ref/tool/#agents.tool.ComputerProvider "ComputerProvider

  
      dataclass
  ") factory should either pass a concrete `Computer` or `AsyncComputer` instance or force the GA selector before sending the request. See [Tools](../tools/#computertool-and-the-responses-computer-tool) for the full migration details.
...
* [`ToolSearchTool`](../ref/tool/#agents.tool.ToolSearchTool "ToolSearchTool

    
        dataclass
    ")
* [`tool_namespace()`](../ref/tool/#agents.tool.tool_namespace "tool_namespace")
* `@function_tool(defer_loading=True)` and other deferred-loading Responses tool surfaces

These features are rejected on Chat Completions models and on non-Responses backends. When you use deferred-loading tools, add `ToolSearchTool()` to the agent and let the model load tools through `auto` or `required` tool choice instead of forcing bare namespace names or deferred-only function names. See [Tools](../tools/#hosted-tool-search) for the setup details and current constraints.
...
Transport selection happens when the SDK resolves a model name into a model instance. If you pass a concrete [`Model`](../ref/models/interface/#agents.models.interface.Model "Model") object, its transport is already fixed: [`OpenAIResponsesWSModel`](../ref/models/openai_responses/#agents.models.openai_responses.OpenAIResponsesWSModel "OpenAIResponsesWSModel") uses websocket, [`OpenAIResponsesModel`](../ref/models/openai_responses/#agents.models.openai_responses.OpenAIResponsesModel "OpenAIResponsesModel") uses HTTP, and [`OpenAIChatCompletionsModel`](../ref/models/openai_chatcompletions/#agents.models.openai_chatcompletions.OpenAIChatCompletionsModel "OpenAIChatCompletionsModel") stays on Chat Completions. If you pass `RunConfig(model_provider=...)`, that provider controls transport selection instead of the global default.
...
If you need prefix-based model routing (for example mixing `openai/...` and `any-llm/...` model names in one run), use [`MultiProvider`](../ref/models/multi_provider/#agents.models.multi_provider.MultiProvider "MultiProvider") and set `openai_use_responses_websocket=True` there instead.
...
* This is the Responses API over websocket transport, not the [Realtime API](../realtime/guide/). It does not apply to Chat Completions or non-OpenAI providers unless they support the Responses websocket `/responses` endpoint.
* Install the `websockets` package if it is not already available in your environment.
* You can use [`Runner.run_streamed()`](../ref/run/#agents.run.Runner.run_streamed "run_streamed

    
        classmethod
    ") directly after enabling websocket transport. For multi-turn workflows where you want to reuse the same websocket connection across turns (and nested agent-as-tool calls), the [`responses_websocket_session()`](../ref/responses_websocket_session/#agents.responses_websocket_session) helper is recommended. See the [Running agents](../running_agents/) guide and [`examples/basic/stream_ws.py`](https://github.com/openai/openai-agents-python/tree/main/examples/basic/stream_ws.py).
...
If you need a non-OpenAI provider, start with the SDK's built-in provider integration points. In many setups, this is enough without adding a third-party adapter. Examples for each pattern live in [examples/model\_providers](https://github.com/openai/openai-agents-python/tree/main/examples/model_providers/).

### Ways to integrate non-OpenAI providers

| Approach | Use it when | Scope |
| --- | --- | --- |
| [`set_default_openai_client`](../ref/#agents.set_default_openai_client "set_default_openai_client") | One OpenAI-compatible endpoint should be the default for most or all agents | Global default |
| [`ModelProvider`](../ref/models/interface/#agents.models.interface.ModelProvider "ModelProvider") | One custom provider should apply to a single run | Per run |
| [`Agent.model`](../ref/agent/#agents.agent.Agent.model "model            class-attribute       instance-attribute   ") | Different agents need different providers or concrete model objects | Per agent |
...
When you want to further configure the model used for an agent, you can pass [`ModelSettings`](../ref/model_settings/#agents.model_settings.ModelSettings "ModelSettings"), which provides optional model configuration parameters such as temperature.
...
A retry policy receives a [`RetryPolicyContext`](../ref/retry/#agents.retry.RetryPolicyContext "RetryPolicyContext
...
* A [`RetryDecision`](../ref/retry/#agents.retry.RetryDecision "RetryDecision

    
        dataclass
    ") when you want to override the delay or attach a diagnostic reason.
...
If you use [`MultiProvider`](../ref/models/multi_provider/#agents.models.multi_provider.MultiProvider "MultiProvider"), pass `openai_strict_feature_validation=True` instead.
...
For [`MultiProvider`](../ref/models/multi_provider/#agents.models.multi_provider.MultiProvider "MultiProvider"), use `openai_buffer_streamed_tool_calls=True`.
...
Reach for a third-party adapter only when the SDK's built-in provider integration points are not enough. If you are using OpenAI models only with this SDK, prefer the built-in [`OpenAIResponsesModel`](../ref/models/openai_responses/#agents.models.openai_responses.OpenAIResponsesModel "OpenAIResponsesModel") path instead of Any-LLM or LiteLLM. Third-party adapters are for cases where you need to combine OpenAI models with non-OpenAI providers, or need adapter-managed provider coverage or routing that the built-in paths do not provide. Adapters add another compatibility layer between the SDK and the upstream model provider, so feature support and request semantics can vary by provider. The SDK currently includes Any-LLM and LiteLLM as best-effort, beta adapter integrations.
