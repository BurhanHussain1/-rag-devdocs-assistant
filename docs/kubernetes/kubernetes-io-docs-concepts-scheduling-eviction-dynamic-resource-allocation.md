...
* [**PodGroup ResourceClaimTemplate**](#workload-resourceclaims): you want
  [PodGroups](/docs/concepts/workloads/podgroup-api/ "A PodGroup represents a set of Pods with common scheduling policy and constraints.") to have
...
[PodGroups](/docs/concepts/workloads/podgroup-api/ "A PodGroup represents a set of Pods with common scheduling policy and constraints.")
...
[ResourceClaim](/docs/reference/kubernetes-api/resource/resource-claim-v1/#ResourceClaimStatus) API reference.
...
FEATURE STATE:
`Kubernetes v1.36 [beta]`(disabled by default)
...
4. **Container exposure.** Files are typically exposed via
   [CDI](/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/ "A CNCF specification for describing device configuration that container runtimes apply when creating containers.") bind-mounts, but other
