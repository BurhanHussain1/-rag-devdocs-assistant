...
* [Horizontal pod autoscaling](/docs/concepts/workloads/autoscaling/horizontal-pod-autoscale/ "Object that automatically scales the number of pod replicas based on targeted resource utilization or custom metric targets.")
...
* `terminationGracePeriodSeconds` - this is not fully implemented in Docker on Windows,
  ...
  The behavior today is that the ENTRYPOINT process is sent CTRL\_SHUTDOWN\_EVENT,
...
For Kubernetes v1.36, operating system compatibility for Windows nodes (and Pods)
is as follows:
