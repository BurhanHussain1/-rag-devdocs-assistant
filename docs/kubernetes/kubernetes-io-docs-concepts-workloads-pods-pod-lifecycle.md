...
[PodStatus](/docs/reference/generated/kubernetes-api/v1.36/#podstatus-v1-core)
...
* [static Pods](/docs/tasks/configure-pod-container/static-pod/) (which are
  managed directly by the kubelet and represented by [mirror Pods](/docs/reference/glossary/?all=true#term-mirror-pod "An object in the API server that tracks a static pod on a kubelet."))
* [force-deleted
...
[init containers](/docs/concepts/workloads/pods/init-containers/ "One or more initialization containers that must run to completion before any app containers run.") run only once (if successful),
during Pod startup.
...
[sidecar container](/docs/concepts/workloads/pods/sidecar-containers/ "An auxilliary container that stays running throughout the lifecycle of a Pod.")) or define container-level override.
...
3. **Pod Status Update**: The Pod's status is updated with a `PodRestartInPlace` condition set to `True`.
   This makes the restart process observable.
4. **Full Restart Sequence**: Once all containers are terminated, the `PodRestartInPlace` condition
...
[`pods/restart-policy/restart-all-containers.yaml`](https://raw.githubusercontent.com/kubernetes/website/main/content/en/examples/pods/restart-policy/restart-all-containers.yaml)![](/images/copycode.svg "Copy pods/restart-policy/restart-all-containers.yaml to clipboard")
...
[PodConditions](/docs/reference/generated/kubernetes-api/v1.36/#podcondition-v1-core)
...
containers. The Pod [conditions](/docs/concepts/workloads/pods/pod-lifecycle/#pod-conditions "A condition represents the current state of a Kubernetes resource, providing information about whether certain aspects of the resource are true.")
...
by the [kubelet](/docs/reference/command-line-tools-reference/kubelet "An agent that runs on each node in the cluster. It makes sure that containers are running in a pod.") on a container.
...
[EndpointSlice](/docs/concepts/services-networking/endpoint-slices/ "EndpointSlices track the IP addresses of Pods for Services.")
