...
for pods in the `Guaranteed` [QoS class](/docs/concepts/workloads/pods/pod-qos/ "QoS Class (Quality of Service Class) provides a way for Kubernetes to classify pods within the cluster into several classes and make decisions about scheduling and eviction.").
...
* **Empty shared pool restriction:** This configuration does not allow pod
  ...
  [`pods/resource/pod-level-resource-managers-empty-shared-pool.yaml`](https://raw.githubusercontent.com/kubernetes/website/main/content/en/examples/pods/resource/pod-level-resource-managers-empty-shared-pool.yaml)![](/images/copycode.svg "Copy pods/resource/pod-level-resource-managers-empty-shared-pool.yaml to clipboard")
